...
//        fill(matrices, this.x, this.y, this.x + width, this.y + this.height, this.isHovered() ? 0x507E0707 : 0x307E0707);
        ...
        graphics.fill(this.x, this.y, this.x + width, this.y + this.height, color);
//        this.renderWidget(matrices, mouseX, mouseY);
//        this.renderWidget(matrices, 0, 0, 0);
        int j = this.active ? 0xFFFFFF : 0xA0A0A0;
        graphics.drawCenteredString(textRenderer, this.getMessage(), this.x + this.width / 2, this.y + (this.height - 8) / 2, j | Mth.ceil(this.alpha * 255.0f) << 24);

        if(this.selected) {
            graphics.fill(this.x, this.y + this.height - 2, this.x + this.width, this.y + this.height,  0xFFFFFFFF);
        }
    }
